    ...
    <div class="content-wrapper">
        ...
        <div id="page-content">
            <div style="display: flex;align-items: center;}">
                ...
                <h3 class="page-header">{{ __('Users list') }}</h3>
                <div><a style="margin-left:500px" href="  {{ route('user.create') }}"></i>{{ __('Add user') }}</a></div>
            </div>
            <table class="table table-bordered" id="danhsach">
                <thead>
                    <tr>
                        ...
                        <th style="width: 250px">{{ __('Email') }}</th>
                        <th style="width: 130px">{{ __('Users name') }}</th>
                        <th style="width: 200px">{{ __('Birtday') }}</th>
                        <th style="width: 200px">{{ __('Avatar') }}</th>
                        <th style="width: 200px">{{ __('City') }}</th>
                        <th style="width: 200px">{{ __('District') }}</th>
                        <th style="width: 200px">{{ __('Wards') }}</th>
                        <th class="hidden-xs hidden-sm"></i> Flag_delele</th>
                ...
                @foreach ($data as $item)
                    <tbody>
                        ...
                        <td><img style="width: 50px" src="{{ asset('/upload/user') }}/{{ $item->avatar }}"></td>
                        <th>{{ $item->province->name }}</th>
                        <th>{{ $item->district->name }}</th>
                        <th>{{ $item->commune->name }}</th>

                        {{-- <th>{{$formattedNum = number_format($item->gia)}}</th> --}}
                        <th style="text-align: center">{{ $item->flag_delete }}</th>
                        <!-- <th>Ps15272</th> -->
                        <td class="text-center">
                            <div class="btn-group">
                                <a style="border-radius: 8px" href="/welcome/{{ $item->id }}" data-toggle="tooltip"
